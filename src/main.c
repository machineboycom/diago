...
static bool weekNumbers;
static bool showFahrenheit;
...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  if(temp_tuple && conditions_tuple && wind_tuple) {
    int kelvin = (int)temp_tuple->value->int32;
    if(showFahrenheit == true){
      
      snprintf(temperature_buffer, sizeof(temperature_buffer), "%dF", (int)(kelvin * 9/5 - 459.67));
    }else{
      snprintf(temperature_buffer, sizeof(temperature_buffer), "%dC", (int)(kelvin - 273.15));
    }
...
static void main_window_load(Window *window) {
  ...
  // Style the weather, choose F or C
  ...
  s_step_layer = text_layer_create(GRect(0, 140, 75, 30));
